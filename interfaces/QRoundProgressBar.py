...
class QRoundProgressBar(QtGui.QWidget):
    ...
    def drawBase(self, p, baseRect):
        bs = self.barStyle
        palette_shadow = self.palette().shadow().color()
        if bs == self.StyleDonut:
            outline_color = QtGui.QColor(0,0,0)
            p.setPen(QtGui.QPen(outline_color, self.outlinePenWidth))
    ...
    def drawValue(self, p, baseRect, value, arcLength):
        ...
        pen_color = self.palette().shadow().color()
        pen_color = QtGui.QColor(0,0,0)
        p.setPen(QtGui.QPen(pen_color, self.dataPenWidth))
        p.drawPath(dataPath)
